...
/**
 * 检查并补全IMU数据，然后存入gvins::imu_buffer_内
 * @param imu
...
bool GVINS::addNewImu(const IMU &imu) {
    ...
    if (imu_buffer_mutex_.try_lock()) {
        ...
        if (imu.dt > (imudatadt_ * 1.5)) {
            ...
            IMU imudata  = imu;               // 插值的IMU数据
            imudata.time = imu.time - imu.dt; // 回到上一个时刻的IMU时间
            while (cnts--) {
...
bool GVINS::addNewGnss(const GNSS &gnss) {
    ...
    if (integration_config_.origin.isZero()) {
        ...
        LOGI << "gnss origin is initialized as " << integration_config_.origin;
...
/**
 * 在系统初始化完成后，保存图像到gvins::frame_buffer_中
 * @param frame
 * @return 是否成功获取到图像数据锁，将数据存入buffer内
 */
        ...
        while (!imu_buffer_.empty()) { // IMU BUFFER
            ...
            { // INS
                ...
                if (gvinsstate_ > GVINS_INITIALIZING) {
                    ...
                    // Only reserve certain INS in the window during
                    // initialization
                ...
                if (gvinsstate_ == GVINS_INITIALIZING) {
                    ...
                    // 仅加入关键帧节点, 而不进行优化
                    // Add new time node during the initialization of the visual
                    // system
    ...
    while (!isfinished_) {
        ...
        if (isgnssobs_ || isvisualobs_) {
            ...
            if (gvinsstate_ == GVINS_INITIALIZING_INS) {
                ...
                // 关键帧数量达到窗口大小, 需要边缘化操作,
                // 并移除最老的关键帧及相关的GNSS和预积分观测, 由于计算力的问题,
